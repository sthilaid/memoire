...
chapitre afin de pouvoir illustrer l'évolution de ceux-ci et de situer
...
produit. Ainsi, le moteur de rendu doit pouvoir être facilement
extensible afin de pouvoir accommoder facilement les nouvelles idées
et les nouveaux concepts à intégrer.
...
Il semble donc que le choix d'un langage permettant facilement de
répondre à ces contraintes pourrait certainement faciliter le
développement de jeux vidéo. Ainsi, des économies substantielles aux
compagnies de l'industrie pourraient être engendrée, rendant plus
accessible le développement de jeux aux plus petites compagnies
...
\textit{VI} ou \textit{Gnu Emacs}. De plus, \Schemelang utilise \emph{la
...
créent des programmes \Schemelang. C'est ce qui constitue la base du
système de macro de \Schemelang. On dit ainsi qu'un programme \Schemelang est
représenté par des expressions symboliques, fréquemment appelées
S-expression. La distinction dans un programme \Schemelang entre une donnée
...
Le standard du langage \Schemelang~\cite{R5RS} est très minimaliste, mais
requiert toutes les fonctionnalités de base requises par un langage de
haut niveau comme \Schemelang. On y spécifie que \Schemelang est un langage typé
...
particularité fait en sorte que \Schemelang est un langage qui peut être
...
(substring (list->string '(\#\textbackslash a \#\textbackslash l \#\textbackslash l \#\textbackslash o \#\textbackslash !)) 
           ...
           3) => \schemeresult{"all"}
  \end{schemecode}
  \caption{Exemples d'utilisation des type de bases de \Schemelang}
...
Le langage \Schemelang est à la base un langage fonctionnel. Cette famille
...
fidèlement au modèle de calcul de Alonzo Church. Le langage \Schemelang est
...
\Schemelang partage le meilleur des deux mondes en laissant la liberté au
...
étant de très mauvais style en \Schemelang.
...
d'éviter ce problème en \Schemelang en générant un nom de variable pour
...
Un exemple plus convaincant d'une macro \Schemelang utilisant la puissance
...
nouvelles constructions à \Schemelang permettant d'exprimer facilement des
...
terminal dans la fonction \scheme{fact}. Le standard du langage \Schemelang
...
Le langage \Schemelang offre la possibilité de réifier automatiquement la
...
L'égalité en \Schemelang entre deux objets appartenant aux types de base du
...
sur des pointeurs et puisque, en \Schemelang, un symbole ou un mot clé ne
possède qu'un instance unique, ce prédicat peut être également utilisé
pour distinguer ces derniers types.

L'opération conditionnelle de base en \Schemelang est effectué par la forme
...
Plusieurs fonctions sont disponible en \Schemelang afin d'effectuer
l'affichage du résultat de l'évaluation d'une expression. Le standard
\Schemelang fourni entre autre les fonctions \scheme{write} et
...
\FloatBarrier
\section{Conclusion}
Cet aperçu des particularités du langage fonctionnel \Schemelang démontrent
...
% deplacee pcq trop grande... mise a la fin de la section
\begin{figure}[h!]
  \begin{verbatim}
class Toto {
...
du langage \Schemelang, les macros \Schemelang ont dû être utilisées. Il en
résulte donc en l'ajout d'un langage spécifique au domaine de la
programmation orientée objet.

Puisque l'expansion de macros \Schemelang se fait lors de la compilation,
...
\Schemelang. Ceux-ci ont été choisis dans le but de donner un accès rapide
...
vecteur est une opération efficace en \Schemelang, nous avons préféré opté
...
de modifier la hiérarchie de classe définie dans un module \Schemelang (un
...
poursuivre son exécution. Puisqu'une telle attente peu varier en
fonction de la plate-forme utilisée, elle introduit un certain
indéterminisme dans le système.
...
coroutines est très importante. Afin de pouvoir facilité l'écriture
modulaire de corps des coroutines, des formes spéciales permettant
d'utiliser un style CPS ont été ajoutées. Ces dernière permettent
l'altération le fil d'exécution d'une coroutine en modifiant la
continuation de son calcul par celui d'une autre coroutine ou d'un
\textit{thunk}. Ainsi, un appel à la forme spéciale
\scheme{(continue-with <corout>)} fera en sorte que la continuation de
la coroutine devienne celle de cette autre coroutine. De manière
similaire, \scheme{(continue-with-thunk!  <thunk>)} utilisera la
fonction passée comme continuation du calcul. La figure
\ref{Corout:cont-mod} présente un exemple utilisant ces deux formes
spéciales. Lorsque la coroutine \scheme{c1} s'exécute, sa continue
change pour celle de \scheme{c2} qui change à sont tour pour la
fonction \scheme{t}. Ainsi, la fin du corps des coroutines \scheme{c1}
et \scheme{c2} ne sera jamais exécutée puisque leurs continuation ont
été altérées.
...
\item \scheme{(recv (<pattern> <body>) ...)} : Cette forme spéciale
  ...
  permettent de facilement exprimer la forme attendue du message et de
  lier des variables locales à des parties du message reçu afin de les
  utiliser dans le corps du patron. Lorsque plusieurs messages reçus
  peuvent correspondre aux motifs donnés, la sélection du message se
  fait en choisissant le premier message reçu qui correspond à au
  moins un patron. Si plusieurs patrons peuvent correspondre à ce
  message, le premier patron (selon l'ordre de définition) sera
  utilisé. Ainsi l'ordre de spécification des motifs est important et
  significatif.
\end{itemize}
...
Afin de pouvoir déterminer si la coroutine possède ou non une valeur
de retour, un symbole unique connu est utilisé afin d'initialiser la
...
l'état \scheme{interruptible?} du champs \scheme{sleeping?} d'une
coroutine est utilisé.
...
\ref{Corout:recv-order} illustre ce comportement.
 ...
 deuxième appel à la macro \scheme{match} est fait afin de pouvoir
 exécuter le corps du patron dans un environnement effectuant les
 liaisons déclarées dans le motif du patron. Il en résulte ainsi une
 légère perte de performances, mais l'optimisation de cette double
 recherche du motif est complexe~\cite{Marc-Erlang}. Ainsi, puisque
...
externes et d'avoir autant que possible de code en \Schemelang. Par contre,
...
Ainsi, les macros et les fonctions d'ordre supérieures de \Schemelang ont
permis de pouvoir exprimer de manière élégante et modulaire les
concepts reliés à la déclarations de machines à états pour les entités
ainsi que les niveau du jeu \textit{Lode Runner}.
