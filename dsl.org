...
(define (invader-controller self)
  (lambda ()
    (wait-for-state state1 'changed-color (row-size (Inv-Controller-row self))
                    (broadcast (row-invaders (Inv-Controller-row self) 'move))
                    (state2))
    
    (wait-for-state state2 'moved (row-size (Inv-Controller-row self))
                    (recv 
                     ('go-down-warning
    ...
    (wait-for-state state3 'changed-color (row-size (Inv-Controller-row self))
