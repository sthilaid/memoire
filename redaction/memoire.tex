...
(write (list 1 'a "a")) => \schemeresult{(1 a "a")\textbackslash~n} 
(println "Bonjour " (+ 1 2) (map (flip + 1) '(1 2 3))) => \schemeresult{Bonjour 3234}
  \end{schemecode}
...
simplement une expression \Schemelang. Finalement, un patron spécial
...
\Schemelang. L'avantage d'utiliser des données comme identificateur
est qu'il est alors possible de créer celui-ci durant
l'exécution. L'utilisation de variables \Schemelang pointant vers une
structure de données, plutôt que des données \Schemelang, offriraient
...
\item \scheme{sleeping-coroutines}: Nombre de coroutines en
  ...
  \scheme{q} et \scheme{time-sleep-q} n'est pas suffisant. En effet,
...
dernière est implantée par une macro \Schemelang qui transforme les
...
externes et à avoir autant que possible de code en \Schemelang. Par
...
\begin{figure}[htb!]
  ...
  \caption{Exemple de fonte utilisée dans \si.}
  \label{Exp:font}
\end{figure}
...
explose, etc... Ainsi, un petit système de simulation par évènements
...
fermetures \Schemelang sans arguments, effectuant le travail à
...
développement de ce système fait l'objet du chapitre \ref{Chap:OO}.
...
peuvent être n'importe quelles expression \Schemelang. Aussi, lors de
...
(define-state-machine level
  ...
  create-new-class?: \#f)
  \end{schemecode}
  \caption{Implantation de la machine à états pour les niveaux de \lr}
...
puisque dans \lr la plupart des objets sont statiques, on obtient une
...
ce qui a été utilisé pour \si. Une fonction générique
