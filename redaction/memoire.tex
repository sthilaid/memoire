...
\scheme{spawn-brother}. Ces fonction sont décrites les sous-sections
qui suivent.
...
devient possible de modulariser les différents aspects d'utilisations
...
un système de coroutine utilisé pour la partie.

La procédure de création de ces derniers est obtenue en démarrant un
nouveau système de coroutine à l'intérieur d'une coroutine
...
l'arborescence de système de coroutines présentement active. Un
...
section doivent être exécutées \emph{par} une coroutine, \ie dans leur
corps.
...
ne sera exécuté uniquement lorsque toutes les autres coroutines prêtes
auront décidé de passer la main à la coroutine suivante. Il est
...
accès à de l'information sur le système de coroutine en cours
...
par les formes équivalentes dans les système Termite~\cite{Termite} et
...
sélection d'un message est complètement déterministe. En effet, la
...
dans la file d'attente des coroutine prêtes. La figure
...
\textit{define-type}. Leurs rôles est de fournir une abstraction
...
système actuel dans le champs \scheme{state-env} d'un objet de
...
message n'est pas disponible, la coroutine attend alors jusqu'à la
...
(recv (salut 'non) 
  ...
  \caption{Patron de filtrage utilisé pour les tests de
    performances. Le message reçu par ce dernier est celui attendu par
    la deuxième branche.}
...
(let* ((c2 (new-corout 'c2 
  ...
  \caption{Test de performance \scheme{ping-server} utilisé pour
    comparé une utilisation réaliste de la réception de messages avec
    filtrage de motifs.}
...
externes et à avoir autant que possible de code en Scheme. Par
contre, les librairies SDL~\cite{SDL} et \textit{OpenGL}~\cite{OpenGL}
ont été utilisées afin de simplifier la gestion d'entrées/sorties, le
...
la description \scheme{((colors: (white green red)) (chars: (0
  1)))}.\\
...
explose, \textit{etc}. Ainsi, un petit système de simulation par évènements
discrets a été développé et utilisé. Ce dernier fut implanté de
manière très simple en utilisant un monceau triant les évènements
ordonnancés de manière croissante en fonction du temps de leurs
arrivée dans la simulation. Ces évènements étaient des \textit{thunk},
fermetures Scheme sans arguments, effectuant le travail à
accomplir par l'évènement. Lorsqu'un évènement se termine,
l'ordonnanceur choisi alors le prochain et appelle le corps de
...
ce qu'elle se fasse appelée dans \scheme{<secs>}
...
ajoutées dans chaque instances, mais il en résulterait un gaspillage
...
par évènements discrets), fonctionne bien pour certains aspects tel
que la description d'animations, mais ne semble pas naturelle pour
d'autres. Ainsi, le flot de contrôle pourrait aussi être amélioré de
manière à exprimer de manière plus naturelle le comportement des
entités du jeu.


\FloatBarrier
\subsection{Version orientée objet} \label{Exp:sp2}
Une nouvelle version de \textit{Space Invaders} fut développée afin de pallier à une
...
fastidieuses de vérifications de types pour la résolution de
...
pour apposer des propriétés aux objets du jeux. La figure
...
d'images dans une fonte. La classe \scheme{invader-ship} sous-classe
ces deux dernières tout en ajoutant les deux champs communs à tous les
types d'\textit{invader}. Cette dernière est elle-même sous-classée
par les trois types d'ennemis existant (à l'exception du vaisseau mère
qui est traité à part). Puisque chaque type d'\textit{invader} possède
sa propre classe, on peut alors spécifier les valeurs des champs de
...
résulte ainsi que l'écriture de ces parties du code sont plus élégante
et solides face aux changements éventuels des structures de données ou
...
dévelop\-pement de \textit{Space Invaders} ont été réutilisées et raffinées pour les besoins
plus grands de \textit{Lode Runner}. Par contre, le flot de contrôle de la logique du
...
Les entités du jeu \textit{Lode Runner} sont sensiblement plus complexes que celles
présentent dans le jeu précédant. En effet, ces dernières possèdent
sont animés et réagissent différemment en fonction du contexte de
celles-ci. Encore une fois, ce sujet est traité de manière
...
(define-macro (define-cyclic-animation name
  ...
  \caption{Forme spéciale utilisée afin de simplifier la définition
    d'animations cycliques d'entité du jeu \textit{Lode Runner}}
...
tous les 60 images rendues, tel qu'illustrer dans la figure
...
Ainsi, les macros et les fonctions d'ordre supérieures de Scheme
