...
fonction de la plateforme utilisée, elle introduit un certain
...
\item \scheme{(recv (<pattern> <body>) ...)} : Cette forme spéciale
  ...
  fait en choisissant dans un premier temps le premier motifs
  possible, et par la suite le premier message répondant à ce
  motif. Ainsi l'ordre de spécification des motifs est important et
  significatif.
\end{itemize}
...
disponible sous la forme \scheme{(after <timeout> <body> ...)}. Ce patron
...
Le filtrage par patron présenté diffère de ceux disponibles dans les
système Termite~\cite{Termite} ou Erlang~\cite{Erlang} par le fait que
la sélection du message reçu se fait prioritairement sur le premier
patron possédant un message valide, contrairement à ces deux autres
systèmes où la sélection se fait plutôt sur le premier message reçu
correspondant à un filtre existant. Il en résulte que l'ordre dans
lequel sont spécifiés les motifs de filtrage donne une priorité
décroissante a ceux-ci. Il en résulte que certains messages peuvent
ainsi être considéré prioritairement à leurs prédécesseurs. Ce
mécanisme est fort utile pour l'implantation d'un jeu vidéo où un
changement d'état du jeu nécessiter des actions immédiates, comme la
mort d'un joueur, par exemple. Toutefois, puisque le réveil de
coroutines en attente sur le temps est indéterministe, il est possible
que l'arrivée de messages prioritaires soit retardée. Un tel cas de
figure résulterait en une exécution différente du système et donc, il
en résulte que la réception de messages est indirectement
indéterministe aussi.
...
Scheme. L'avantage d'utiliser des données comme identificateur est
qu'il est alors possible de créer celui-ci durant
l'exécution. L'utilisation de variables Scheme pointant vers une
structure de donnés, plutôt que des données Scheme, offriraient de
...
dernière est implantée par une macro Scheme qui transforme les patrons
