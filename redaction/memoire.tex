...
%% Si cette option est activee, l'espace a droite des pages impaire
%% (ou a gauche des pages paires si le documents utilise un style
%% twoside) est reduit a un pouce.
%\usepackage[rmargin=1in]{geometry}
...
Scheme for the developpement of their titles. The use made of
Scheme alowed us to perform various high-level abstractions that
...
Scheme sont discutées plus en détails dans le chapitre
\ref{Chap:Scheme}.
...
caractéristiques de Scheme et du compilateur Gambit-C, ainsi que
les besoins au niveau du dévelop\-pement de jeux vidéo. Par la suite,
...
dévelop\-pement de ces derniers. Par la suite, une introduction traitant
de programmation fonctionnelle, du langage de programmation
Scheme et des outils de dévelop\-pement disponibles, en particulier
...
dévelop\-pement de jeux vidéo. On y présente la programmation orientée
...
de jeux aux contenus diversifiés. Parmi ceux-ci, \textit{Space Invaders}(1978) et
...
dans le dévelop\-pement de jeux, mais plutôt de rendre disponible les
...
Le dévelop\-pement de jeu vidéo implique une programmation sous des
...
Une autre contrainte importante reliée au dévelop\-pement de jeu est la
...
Le dévelop\-pement de logiciel favorisant la modularité est une qualité
standard en génie logiciel, mais elle vient très bien s'appliquer au
dévelop\-pement de jeux modernes. En effet, ces derniers sont des
...
réduire le coût de dévelop\-pement de tels projets.
...
qualité est importante pour le dévelop\-pement de jeu puisqu'un tel
...
petites accélérations d'un cycle de dévelop\-pement pourraient avoir un
impact sur le travail de beaucoup de gens et se traduire ainsi en de
grandes économies sur les coûts de dévelop\-pement.
...
par la suite. Ainsi, afin d'optimiser les efforts de dévelop\-pement
...
programmation Scheme comme outil principal de dévelop\-pement de
jeux vidéo. Il s'agit d'un langage de très haut niveau qui semble être
un candidat idéal pour permettre un dévelop\-pement efficace de jeux
...
évolué qui permet d'augmenter Scheme avec des langages
...
pour donner naissance au standard Common Lisp. En 1984, Guy Steele publia
...
dialectes lui ayant donné naissance. Ainsi, Common Lisp possède beaucoup de
...
Lisp appelé Scheme~\cite{SCHEME_ORIGINS}. L'origine du
dévelop\-pement de Scheme provient de la tentative de compréhension
...
programmeurs Scheme. De plus, le langage est accompagné d'une
...
efficacement en utilisant des outils de dévelop\-pement performant tel
\textit{VI} ou \textit{Gnu Emacs}. De plus, Scheme utilise la
...
programmes Scheme. C'est ce qui constitue la base du système de
macro de Scheme. On dit ainsi qu'un programme Scheme est
...
Le standard du langage Scheme~\cite{R5RS} est très minimaliste,
mais possède toutes les fonctionnalités de base requises par un
langage de haut niveau comme Scheme. On y spécifie que
Scheme est un langage typé dynamiquement, \ie que les
...
Scheme est un langage qui peut être interprété ou compilé. Les
...
(substring (list->string '(\#\textbackslash{}a \#\textbackslash{}l \#\textbackslash{}l \#\textbackslash{}o \#\textbackslash{}!)) 
           ...
           3) => \schemeresult{"all"}
  \end{schemecode}
  \caption{Exemples d'utilisation des type de base de Scheme}
...
Scheme permet l'implantation d'un système de macro très puissant, dit
...
en Scheme en générant un nom de variable pour \scheme{loop} qui
...
création des fonctions auxiliaires d'accès aux membres, à la
détermination des membres hérités, \textit{etc}.
...
disponibles~\cite{GC_REVIEW1}~\cite{GC_REVIEW3}.
...
gé\-né\-ra\-tion\-nels~\cite{GEN_GC}. Brièvement, cette technique divise la
...
gé\-né\-ra\-tion\-nels nécessitent l'arrêt du programme durant l'opération
récupération, qui peut, dans le pire des cas, s'effectuer sur toute la
mémoire. Ainsi, les gestionnaires de mémoires gé\-né\-ra\-tion\-nels
...
gé\-né\-ra\-tion\-nels semblent un bon choix pour la programmation de
...
Le langage Scheme est un langage à typage dynamique, faisant
...
Scheme fourni entre autres les fonctions \scheme{write} et
...
Le langage Scheme étant très simple, il est essentiel de créer
...
Scheme ont été développées dans le but de répondre spécifiquement
...
Scheme est celui de la programmation orientée objet. Celui-ci
...
Le langage Common Lisp~\cite{CLISP} possède un système orienté objet, le
...
Scheme Gambit-C. La plupart sont inspirés de la puissance
...
objets sur la présence de fonctionnalités utiles pour le dévelop\-pement
de jeux vidéo.
...
Common Lisp~\cite{CLOS}, le système objets développé possède des
...
notre système objets puisse offrir le plus de puissance expressive
...
exemple, la résolution de collision est un comportement qui dépend des
deux entités impliquées et donc l'utilisation d'une fonction générique
pour effectuer la résolution du travail à accomplir se fait de manière
très naturelle.
...
fonctionnalité n'est pas nécessaire pour le dévelop\-pement de jeu.
...
des vecteurs Scheme. Ceux-ci ont été choisis dans le but de
...
vecteur est une opération efficace en Scheme, nous avons préféré
...
modifier la hiérarchie de classe définie dans un module Scheme
...
\begin{table}
  ...
  \caption{Temps d'exécution (en secondes) de 1~000~000 itérations de
    ...
    orientée objets développé avec d'autres systèmes objets existants}
  \label{OO:bench}
\end{table}
...
Ainsi, les performances obtenus se comparent très bien à Gambit-C
lui-même pour la création d'objets et l'accès au membres de
ceux-ci. Aussi, la performance d'appels de fonctions générique, quoi
...
faire, le dévelop\-pement du système objets présenté dans le chapitre
...
système objets en plus du petit langage de définition d'animations
...
La performances des jeux développés a été évaluée sur aspects. Ces
derniers mesurent l'efficacité des jeux sur l'utilisation de
ressources (processeur et mémoire), ainsi que sur l'impacte du système
de gestion automatique de mémoire. Le tableau \ref{Exp:perf} illustre
les résultats obtenus face à ces points de comparaison. Les résultats
présentées dans ce tableau ont été obtenus en jouant durant environ
une minute aux versions compilées des jeux.
...
\begin{table}
  ...
  \begin{tabular}{ccc}
    ...
    \hline \hline
    Utilisation du processeur & 40~\% & 70~\%\\
    Nombres d'images par secondes & 29.8 & 68.2\\
...
On constate que le jeu \textit{Space Invaders} ne présente pas de
bonnes performances quant à la gestion du processeur. Le taux de
rafraîchissement faible présenté par celui-ci est la conséquence de
l'utilisation de systèmes de flot de contrôles expérimentaux. En
effet, l'ordonnancement fut difficile à bien orchestrer et c'est
principalement pour cette raison que l'utilisation de simulations à
événements discrets et de coroutines a été abandonnée.

Toutefois, le jeu \textit{Lode Runner} présente des performances très
intéressantes. L'utilisation du processeur a été beaucoup améliorée en
utilisant une technique de flot de contrôle plus traditionnelle. On
observe un pourcentage d'utilisation du processeur à environ 70~\%
tout en donnant un taux de rafraîchissement de plus de 80 images par
secondes. 
...
minimes. De plus, \textit{Lode Runner} présente un taux de
rafraîchissement répondant bien aux attentes nécessaires à la fluidité
des jeux.

Ainsi, il serait maintenant intéressant de voir quel serait l'impact
de l'implantation en Scheme d'un jeu de qualité commerciale afin
...
de celles offertes en Scheme. Le tableau \ref{Rev:lang-comp}
...
sous-ensemble du langage Scheme. La présence de gestion de
...
raffiner le \textit{gameplay}. Scheme offre beaucoup de
...
pour le dévelop\-pement de moteurs de jeu sur des plate-formes modernes
...
système objets et des coroutines. Ce dernier fut utilisé pour
...
envergure afin de pousser plus loin l'aspect dynamique du langage par
...
des performances qu'offre Scheme pour le dévelop\-pement de jeux
vidéo.
