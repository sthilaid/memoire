...
\item \scheme{(? [timeout: <sec>])} : Réception du premier message
  ...
  optionnelement spécifié afin de limiter l'attente faite. Si
  plusieurs messages sont disponible, le premier arrivé sera alors
  retourné, afin de respecter une politique équitable de réception de
  messages.
...
\item \scheme{(?? <predicat> [timeout: <secs>])} : Réception du
  ...
  après un délais de temps optionnlement spécifié.
...
permet d'émuler la base de système de programmation réactive. Les
fonctions de gestion et d'utilisation de listes de diffusions sont:
