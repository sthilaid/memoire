...
d'algorithmes de manière très générique
...
De telles macros comportes beaucoup de problèmes potentiels. Non
...
macro très puissant, un système de macro procédural. Ainsi, une macro
Scheme est une fonction Scheme dont les paramètres \emph{ne sont pas
  évalués} avant d'être passés au corps de la fonction. Il en résulte
ainsi que le code étant en position d'argument est considéré par la
macro comme étant des données Scheme (listes, symboles, nombres,
etc...). Puisque la macro Scheme est une fonction, elle possède toute
la puissance du langage à sa disposition.
...
macro s'attend à avoir un nombre en paramètre, mais elle reçoit
plutôt de ce cas ci la liste \scheme{(+ 1 2)}.
