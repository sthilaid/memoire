...
Puisque les fonctionnalités offertes ne répondaient pas bien aux
besoins de la cause, un nouveau système a été développé. Cette
approche se marie bien avec la philosophie de Scheme qui incite les
programmeurs à utiliser l'expressivité de Scheme pour développer leurs
propres outils lorsque ceux disponible ne sont pas
satisfaisant. Ainsi, les programmes Scheme ne se plient pas aux
contraintes de ces systèmes pour arriver à être exprimer de la manière
désirée. Ce chapitre vise donc à présenter un système de
\textit{threads} \emph{coopératifs} qui vient pallier aux lacunes que
présente le système offert par Gambit-C en ce qui à attrait au
développement de jeux vidéo.

Un net avantage lorsque le système est directement implanté par
l'utilisateur est qu'il répond sur mesures aux demandes de ce dernier
et donc l'utilisateur peut utiliser une syntaxe simple d'utilisation
tout en conservant un contrôle fin sur le comportement du système.

Ce chapitre a pour but de décrire le langage de programmation de
\textit{threads} coopératifs créé et d'en expliquer le
fonctionnement. 
...
fonction \scheme{spawn-brother} décritent plus bas.
...
(let* ((c1 (new corout 'c1 (lambda () 1)))
       (c2 (new corout 'c2 (lambda () 2)))
       (c3 (new corout 'c3 (lambda () 3)))
       (c4 (new corout 'c4 (lambda () 1)))
       (c5 (new corout 'c5 (lambda () 2)))
       (c6 (new corout 'c6 (lambda () 3)))
       (timer (start-timer! 1.0))
...
\subsubsection{terminate-corout, kill-all!}
\todo{Terminaison de coroutines. }

\subsubsection{super-yield, super-kill-all!} 
